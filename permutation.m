...
%M=floor(0.040*fs/shiftT);	% 波形の加算範囲が40msecになるようにMを決める。
%if mod(M,2)==1, M=(M-1)/2;
%else M=(M-2)/2;
%end
M=14;				% ここでは14に固定（高速化のため）
...
sim(1,1)=99;			% 直流成分の値を仮に置く

% iteration
for k=2:NFFT/2+1,	% 計算中の周波数binナンバー、直流成分k=1は除く
...
for m=1:NFFT/2+1,
	...
	minbin(m)=b;
	sim(b)=100;	% sim(1,1)=99としたので、直流成分は一番最後に計算されるようになっている
...
%	R = [yenv1*env1' yenv1*env2'; yenv2*env1' yenv2*env2'];
%	[a b] = max([R(1,1)^2+R(2,2)^2;R(1,2)^2+R(2,1)^2]);
